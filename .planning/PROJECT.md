...
- ✓ **Workspace & design-system foundation** — Phase 01: pnpm workspace (`packages/design` + `packages/app` skeleton), `DESIGN.md` → Tailwind v4 `@theme` via `gen-theme.mjs`, Ladle rendering on the real Vite/Tailwind stack, toolchain (`pnpm check`) green. Dark-only base backdrop is token-painted (generated `@layer base { html }` from `--color-bg-0`). The full UIKIT component library remains Active (phases 2+).
...
| Base backdrop painted on `html` from `--color-bg-0` (generated `@layer base`) | A `bg-bg-0` utility on the Ladle provider was tree-shaken (outside `@source`), making the dark base incidental; painting `html` from the token guarantees the dark-only base independent of utility scanning | ✓ Phase 01 (quick 260620-q5q) |
...
---
*Last updated: 2026-06-20 after Phase 01 (workspace & design-system foundation complete)*
